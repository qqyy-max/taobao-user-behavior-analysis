...
class LLMClient:
    ...
    def __init__(self, provider: str):
        ...
        if provider == "deepseek":
            ...
            if not api_key:
                raise ValueError("未检测到 DEEPSEEK_API_KEY。请在 src/config.py 中填写并保存您的 DeepSeek API Key。")
            ...
            if not api_key:
                raise ValueError("未检测到 ANTHROPIC_API_KEY。请在 src/config.py 中填写并保存您的 Anthropic API Key。")
            import anthropic
...
def load_system_prompt() -> str:
    """
    加载 Analyst System Prompt。
    v2.0: 优先使用 agent/prompts/analyst.md（增强版），
          并拼接 agent/metrics_dictionary.md 作为口径约束。
          fallback 到 src/prompts/system.md + metrics.md。
    """
    prompt_dir = Path("src/prompts")
    agent_prompt_dir = Path("agent/prompts")
    parts = []

    # v2.0: 优先加载 agent/ 下的增强版 prompts
    analyst_md = agent_prompt_dir / "analyst.md"
    if analyst_md.exists():
        parts.append(analyst_md.read_text(encoding="utf-8"))
    else:
        # fallback: src/prompts/system.md
        p = prompt_dir / "system.md"
        if p.exists():
            parts.append(p.read_text(encoding="utf-8"))

    # 拼接 agent/metrics_dictionary.md 作为口径约束
    metrics_dict = Path("agent/metrics_dictionary.md")
    if metrics_dict.exists():
        metrics_text = metrics_dict.read_text(encoding="utf-8")
        # 截取关键部分避免 prompt 过长（取前 4000 字符）
        parts.append("\n\n---\n\n## 指标口径约束（来自 metrics_dictionary.md）\n\n" + metrics_text[:4000])

    # fallback: src/prompts/metrics.md
    p = prompt_dir / "metrics.md"
    if p.exists() and not metrics_dict.exists():
        parts.append(p.read_text(encoding="utf-8"))
